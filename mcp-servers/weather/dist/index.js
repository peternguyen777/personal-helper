...
async function getAustralianWeather(city) {
...
async function getInternationalWeather(city) {
    const coords = CITY_COORDS[city.toLowerCase()];
    if (!coords) {
        throw new Error(`Unknown city: ${city}. Supported cities: ${Object.keys(CITY_COORDS).join(", ")}`);
    }
...
async function getWeather(city) {
    ...
    throw new Error(`Unknown city: ${city}. Supported Australian cities: ${Object.keys(BOM_STATIONS).join(", ")}. Supported international cities: ${Object.keys(CITY_COORDS).join(", ")}`);
}
// Map day names to days ahead (0 = today)
function parseDayToDaysAhead(day, timezone) {
...
async function getForecast(city, day) {
    const normalizedCity = city.toLowerCase().trim();
...
server.tool("get_weather", "Get current weather conditions for a city. Returns temperature, feels-like, humidity, rain chance, UV index, wind speed, and conditions.", {
    city: z
        .string()
        .describe("City name (e.g., 'Sydney', 'Tokyo', 'London'). Defaults to Sydney if not specified."),
}, async ({ city }) => {
    try {
